...
import { fetchClusters, fetchClusterById, createCluster, updateCluster, deleteCluster, type SellerPlans, type SellerPlanConfig, } from "../api";
import { fetchColonies, type Colony } from "../lib/coloniesApi";
import { CreateClusterModal } from "../CreateClusterModal";
import { updateSellerPlans } from "../api"; 
...
const ClustersPage: React.FC = () => {
  ...
  const [savingPlans, setSavingPlans] = useState(false);
    ...
    setSellerPlansForm(
      normalizeSellerPlans(sp)
    );
            // 2. Obtener colonias reales del cluster
    ...
    // Por ahora usamos prompt sencillo; luego puedes reemplazarlo por un modal bonito
    const newName = window.prompt("Nuevo nombre del cluster", selectedCluster.name);
    if (!newName) return;

    const newCity = window.prompt("Nueva ciudad del cluster", selectedCluster.city);
    if (!newCity) return;
    ...
    try {
      ...
      // Actualizamos el detalle en memoria
...
Esto también desactivará sus colonias.`
    ...
    try {
      ...
      // Volvemos a la lista y refrescamos
    ...
    >
      ...
      >
        <div>
          ...
          >
            Configura cuánto pagará un vecino para poder publicar productos en este
            cluster.
          </p>
        </div>
      ...
      >
        ...
        <SellerPlanCard
          ...
          subtitle="El vecino puede publicar 3 producto3."
          plan={sellerPlansForm.threeProducts}
          onToggleEnabled={(v) =>
            handleChangePlanField("threeProducts", "enabled", v)
          }
          onChangePrice={(v) =>
            handleChangePlanField("oneProducts", "price", v)
          }
        />
